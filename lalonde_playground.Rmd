...
直接看数据，显得 treat (就业培训) 反而对 re78 (收入) 产生了副作用。平均来看，treat导致re78下降635美刀。
...
使用MatchIt的默认参数进行配平。可以看到配平后一部分对照组的样本被去掉了。之前在Coursera上看A Crash Course in Causality的时候，prof说SMD<0.1认为可以接受，SMD在[0.1,0.3]之间就要很谨慎了。而这里black和hispan配平后的SMD都远远超过了0.3。
...
for (v in balance.check.col) {
  ...
  fml <- paste0(v, "~treat")
  # print(summary(glm(fml, data=lalonde.copy, weights=pscore)))
  print(coef(summary(glm(fml, data=lalonde.copy, weights=pscore)))[2,])
}
# regression with controls on propensity score screened data set
print("Causal Effect:")
print(summary(glm(re78 ~ treat + age + educ + black + hispan + nodegree + married + re74 + re75, data = lalonde.copy, weights = pscore)))
